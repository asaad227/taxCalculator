export function taxCalculator(year, income){
    const rates = {
         basicRate: 0.2,
         higherRate: 0.4,
         additionalRate: 0.45
     }
         let totalTax = 0;
         if (year === 1) {
             if (income > 12571 && income < 50270) {
                 totalTax = (income - 12570) * rates.basicRate;
             } else if (income > 50270 && income < 125140) {
                 totalTax = (income - 50270) * rates.higherRate + 37700 * rates.basicRate;
             } else if (income > 125140) {
                 totalTax = (income - 125140) * rates.additionalRate + 74870 * rates.higherRate + 37700 * rates.basicRate;
             }
         }
         return totalTax;
     
 }
 
export function nationalInsurance(year, income){
     const rates = {
         basicRate: 0.12,
         higherRate: 0.02,
         additionalRate: 0.02
     }
         let totalNi = 0;
         if (year === 1) {
             if (income > 12570 && income <= 50250) {
                 totalNi = (income - 12570) * rates.basicRate;
             } else if (income > 50250 && income < 150000) {
                 totalNi = (income - 50250) * rates.higherRate + 40500 * rates.basicRate;
             } else if (income > 150000) {
                 totalNi = (income - 150000) * rates.additionalRate + 100000 * rates.higherRate + 40500 * rates.basicRate;
             }
         }
         return totalNi;
 
 }
 
  
